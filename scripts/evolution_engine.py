...
def apply_param_to_file(file_path: Path, param_id: str,
    ...
    if param_id.startswith("auction_weight_"):
        ...
        old_str = f"'{dim}': {old_value}"
        new_str = f"'{dim}': {new_value}"
        if old_str in content:
            content = content.replace(old_str, new_str)
        else:
            pattern = re.compile(rf"'{dim}':\s*{re.escape(str(old_value))}")
        ...
        if "gainer_min_pct" in param_id:
            old_s = f"min_pct: float = {old_value}"
            new_s = f"min_pct: float = {new_value}"
            content = content.replace(old_s, new_s)
        elif "gainer_top_n" in param_id:
            old_s = f"top_n: int = {int(old_value)}"
            new_s = f"top_n: int = {int(new_value)}"
            content = content.replace(old_s, new_s)
