...
| ⭐ **P0** `--fast` 模式下 `_prefetch_indicators` 不提取 Baostock peTTM/pbMRQ → fund 全 30 (v9.0) | **致命 bug (2026-06-09 发现)**：`--fast` 跳过 tushare PE/PB/市值后，`_prefetch_indicators` 只从 Baostock bars 提取 `close`/`volume`/`ma20`，**不提取** `peTTM`/`pbMRQ`。导致 `_indicators[code]['pe']` 恒为 0 → `_score_fund` 中 `if pe > 0:` 永远 False → fallback 到 `return 30` → 8 只推荐全同分。**修复**：在 `_prefetch_indicators` 的 bar 遍历中，从 `bars[-1]` 提取 `peTTM`→`ind['pe']`、`pbMRQ`→`ind['pb']`。修复后 fund 得分从全 30 变为 35.4~78.2（真实 PE 驱动）。**诊断**：`python3 -c "import json; d=json.load(open('scripts/data/daily_pool.json')); scores=[r['factor_scores']['fund'] for r in d['recommendations']]; print(f'unique={len(set(scores))}, values={scores}')"` — unique==1 且全 30=RCPA。详见 `references/recommender-fast-pe-bug.md`。 |
| ⭐ **P0** `_guess_sector` 全局公告污染 → sector 全标"金融" (v9.0) | **致命 bug (2026-06-09 发现)**：`_guess_sector` Priority 2 遍历 KB **全部** 222 条公告做关键词匹配，不按 code 过滤。当天 KB 前 3 条公告标题含"证券"→命中 SECTOR_KEYWORDS 的"金融"→`sector_hits['金融']` 碾压所有其他板块→无论天汽模还是TCL科技全标"金融"。**修复**：加 `entry_code = str(entry.get('code', '')); if entry_code != code: continue` 仅匹配该股自己的公告。修复后 sector 正确变为"综合"（当 tushare classify_industry 无数据时）。注意：该股自己的公告若含"证券"仍会命中"金融"（如天汽模公告标题含"华泰联合证券"）——这是 SECTOR_KEYWORDS 匹配过于宽泛的衍生问题，优先级低于修复前。 |
| ⭐ no_agent cron timeout 120s 缓存不更新 — config 改 300 未生效 (v9.0) | **根因 (2026-06-09 验证)**：`config.yaml` `cron.script_timeout_seconds: 300` 已设但 `no_agent` cron job 仍用 120s。hermes 调度器可能缓存了 job 创建时的 timeout 值，config 后续修改不追溯更新已有 job。**修复**：将 cron job 从 `no_agent` 脚本模式转为 LLM 驱动模式 — 移除 `script` 字段和 `no_agent: true`，改用 `prompt`（执行 `stock_recommender.py --top 8 --fast`）+ `enabled_toolsets: ["terminal","file"]`。LLM 模式 timeout 不受 120s 限制。手动验证：`timeout 300 python3 stock_recommender.py --top 8 --fast` 确认脚本在 300s 内完成。 |
...
| 🆕 **P2** reflection_log pool_rate 假阴性——池中股涨6%+仍算0% | **发现 (2026-06-09)**：301042 安联锐视收盘 +6.96% 在推荐池中，但 reflection_log 记录 pool_rate=0%、0/50。疑似原因：(1) review.py 在盘前/盘中计算捕获率，收盘数据未更新，(2) 涨幅榜和推荐池的匹配逻辑存在代码名/日期窗口偏差。**诊断**：对比 daily_pool 的 codes × `get_top_gainers()` 的实际涨幅股 codes 求交集。**暂不影响进化判断，但修复后可使 pool_rate 更准确反映推荐引擎真实表现。** |
...
| ⭐ LLM复盘 review_diagnosis.json 写入路径 | skill 文档引用 `~/scripts/kb/review_diagnosis.json`，但实际 v1.0 系统的写入路径是 `<workspace>/data/data/kb/review_diagnosis.json`（双层 data 嵌套）。写入前先用 `find` 定位实际 kb 目录。同理，`holdings.json` 在 `<workspace>/` 根目录而非 `data/` 下。LLM 复盘 cron 启动时第一步永远是定位真实工作区路径。 |\n| ⭐🆕 **P0** evolution_engine 读取路径 ≠ 写入路径 (v2.0) | **致命发现 (2026-06-09)**：evolution_engine.py v2.0 从 `DATA_DIR / "kb" / "review_diagnosis.json"` 即 `scripts/data/kb/review_diagnosis.json` 读取诊断（`DATA_DIR = SCRIPT_DIR / "data"`）。但 skill 和 LLM 复盘指引写的是 `scripts/kb/review_diagnosis.json`。**写入 `scripts/kb/` → 引擎永远读不到** → 文工团诊断的 rule_changes 永不生效。**修复**：必须同时写入两个路径，或优先写 `scripts/data/kb/review_diagnosis.json`。**验证**：`grep -n "DATA_DIR\|review_diagnosis" evolution_engine.py \| head -5` → 确认引擎实际读取路径。 |\n| ⭐🆕 **P1** 进化引擎参数建议±20%硬约束——分步递进 | **经验 (2026-06-09)**：`涨幅数量 50→80`（60%）被引擎拒绝，必须先 50→60（20%）作为第一步。建议参数变更前先计算 `|新值-当前值|/当前值 ≤ 0.20`。大变更分步：50→60→72→86（3天渐进）。需在 `change` 字段写明分步目标（如"第一步50→60，目标80"）。 |
...
| ⭐🆕 **LLM复盘·观测期不回建议** (v9.1) | **根因**：进化引擎昨日已落地的参数变更（如涨幅阈值6→5%、涨幅数量50→60），今日的观测数据尚未反映效果。LLM复盘看到相同的 pool_rate=0%，容易再次建议同样的参数变更→引擎判定为 noop 或产生无意义的叠加。**正确做法**：诊断时先检查 `evolution_log.json` 最近1-2条记录的 `changes_applied`，确认哪些参数刚被调整。处于观测期的参数在 module_scores 中标注"⏳观测中（昨日已调）"而不是"warn"，在 rule_changes_suggested 中写 `change` 为 "已执行，观测中" + `confidence: high`。观测期=1个交易日，次日在 reflection_log 中验证效果。 |
| ⭐🆕 **LLM复盘·execute_code批量数据采集** (v9.1) | **技术**：LLM复盘第一步"收集数据"涉及 ~15 个数据源。逐只用 terminal/read_file 需要 15+ 次工具调用。**推荐模式**：将所有数据采集打包进一个 `execute_code` Python 脚本——利用 `from hermes_tools import terminal, read_file` 做内部调度，`json.load` 批量解析，最终 `print(json.dumps(results))` 一次性返回。效率提升 ~10x。**注意事项**：(1) 须用绝对路径（`/home/pc/.hermes/profiles/xiaohong/...`），因为 `$HOME` 被 profile 覆盖；(2) 单个 execute_code 脚本控制在 50KB 以内；(3) 异常用 `try/except` 兜底确保部分缺失不阻塞整体采集。 |
| ⭐🆕 **议会日志健康检查空壳污染** (v9.1) | **症状**：`parliament_log.json` 中大量条目为 rounds=0、decision={}、bull_strength=bear_strength=0 的空壳。**根因**：`system_health_check.py` 在检查议会数据新鲜度时，追加空壳条目到 parliament_log。**诊断**：`meaningful = [e for e in plog if e.get('bull_signals',0) > 0 or e.get('bear_signals',0) > 0]` → `len(meaningful)` 应 ≥1。**修复**：健康检查改为只读不写 parliament_log，或写入独立文件。议会新鲜度检测只看有意义条目的时间戳，忽略空壳。 |
...
| ⭐🆕 **进化引擎双源消费：review_diagnosis + auction_diagnosis** (v9.1) | **发现**：`evolution_engine.py` 不仅消费 `review_diagnosis.json` 中的 `rule_changes_suggested`，还**同时消费 `auction_diagnosis.json`** 中的竞价五维权重建议。引擎 --dry-run 输出中会显示 "来源: auction_diagnosis" 的条目。**影响**：竞价诊断中的权重调整（price_slope/volume_accel/imbalance/premium/sector_dev）会被引擎自动落地到 `auction_features.py`，无需在 review_diagnosis 中重复建议。**验证**：引擎输出中 `💾 已写入 auction_features.py` 即为生效。 |
...
## 场外个股期权交易 (OTC Options)

> 🚨 场外个股期权无公开 API，不走 akshare/tushare 数据管线。盈亏计算依赖标的股价（从 `get_stock_realtime` 获取）。期权消息通过**独立群聊窗口**推送（与现货消息分离）。

### 产品结构：香草看涨分成 (OTC Call + P&L Split)
...
**关键认知**：这不是标准香草期权（最大亏损=权利金）。这是杠杆做多+收益分成结构。保证金（10万）是真正的风险资金，亏光即强制平仓。

### 止损/风控规则（按保证金健康度）

与现货不同，期权不按 MA20 或技术位止损：
...
**上涨**：`gross_profit = change_pct × notional` → `total_profit = gross_profit - option_fee` → `personal_pnl = total_profit × 0.70`

**下跌**：`loss = |change_pct| × notional` → `margin_consumed = min(loss, margin)` → `personal_pnl = -(margin_consumed + option_fee)`

**微涨不覆盖期权费**：若 `gross_profit < option_fee`（如涨0.5%时¥5,000 < ¥8,000），total_profit为负，个人承担差额，券商不分成。

> 完整盈亏场景示例见 → `references/otc-call-pnl-examples.md`（4场景：大涨+15%、下跌-5%、暴跌-9%强平、微涨+0.5%）
...
| 算杠杆用 100万/8000=125x | 实际杠杆 = 100万/10万 = 10x |
...
- `references/llm-review-v2-workflow.md` — 🆕 v9.1 LLM复盘 v2.0 工作流模式（execute_code批量采集+观测期协议+模块诊断速查）
